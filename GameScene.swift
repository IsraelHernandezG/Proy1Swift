...
class GameScene: SKScene, SKPhysicsContactDelegate {
        ...
        //Direccion Personaje
        var direccionPersonaje = ""
        var movement = "false"
        // Controles de la fisica
        var velocidadXp = 10.0
        var velocidadXm = 10.0
        var velocidadYp = 10.0
        var velocidadYm = 10.0
        ...
        var bandera4 = 0
        // Texturas jugador
        let texturePlayerN = SKTexture(image: UIImage(named: "nakedNIdle1")!)
        let texturePlayerNE = SKTexture(image: UIImage(named: "nakedNIdle1")!)
        let texturePlayerE = SKTexture(image: UIImage(named: "nakedEIdle")!)
        let texturePlayerSE = SKTexture(image: UIImage(named: "nakedEIdle")!)
        let texturePlayerS = SKTexture(image: UIImage(named: "nakedSIdle")!)
        let texturePlayerSW = SKTexture(image: UIImage(named: "nakedSIdle")!)
        let texturePlayerW = SKTexture(image: UIImage(named: "nakedWIdle")!)
        let texturePlayerNW = SKTexture(image: UIImage(named: "nakedWIdle")!)
        ...
        //Texturas de los botones de menu
         ...
         let textureExitButton = SKTexture(image: UIImage(named: "menuButton")!)
        let textureSaveButton = SKTexture(image: UIImage(named: "menuButton")!)
        let textureInventoryButton = SKTexture(image: UIImage(named: "menuButton")!)
    ...
    func walkPlayer() {
        ...
        for i in 1...8 {
            let playerTextureName = "naked"+direccionPersonaje+"\(i)"
            walkFrames.append(playerWalk.textureNamed(playerTextureName))
        }
    ...
    func readFile() -> NSString{
        ...
        let level1Dir = "/Users/Javi/Downloads/iRPG-master/iRPG/Niveles.xcassets/nivel_1.txt"
        ...
        for touch in touches {
            ...
            if let name = touchedNode.name
            {
                if name == "Menu"{
                    ...
                    //Se genera el menu y se eliminan los pasados si es que hay
                    
                    inventoryButton.removeFromParent()
                    saveButton.removeFromParent()
                    exitButton.removeFromParent()
                    saveButton = SKSpriteNode(texture: textureMenuButton)
                    saveButton.name = "Save"
                    saveButton.zPosition = 3
                    saveButton.xScale = 2
                    saveButton.yScale = 2
                    saveButton.position = CGPoint(x: self.frame.maxX-650, y: self.frame.maxY-400)
                    cam!.addChild(saveButton)
                    inventoryButton = SKSpriteNode(texture: textureMenuButton)
                    inventoryButton.name = "Inventory"
                    inventoryButton.zPosition = 3
                    inventoryButton.xScale = 2
                    inventoryButton.yScale = 2
                    inventoryButton.position = CGPoint(x: self.frame.maxX-650, y: self.frame.maxY-300)
                    cam!.addChild(inventoryButton)
                    exitButton = SKSpriteNode(texture: textureMenuButton)
                    exitButton.name = "Exit"
                    exitButton.zPosition = 3
                    exitButton.xScale = 2
                    exitButton.yScale = 2
                    exitButton.position = CGPoint(x: self.frame.maxX-650, y: self.frame.maxY-200)
                    cam!.addChild(exitButton)
                    ...
                    self.inventoryButton.run(SKAction.setTexture(textureMenuButtonPressed))
                }else if name == "Save"{
                    self.saveButton.run(SKAction.setTexture(textureMenuButtonPressed))
                }else if name == "Exit"{
                    self.exitButton.run(SKAction.setTexture(textureMenuButtonPressed))
                    inventoryButton.removeFromParent()
                    saveButton.removeFromParent()
                   exitButton.removeFromParent()
                }
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        self.saveButton.run(SKAction.setTexture(textureMenuButton))
        self.inventoryButton.run(SKAction.setTexture(textureMenuButton))
        self.exitButton.run(SKAction.setTexture(textureMenuButton))
